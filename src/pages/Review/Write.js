...
export default class Write extends Component {
  ...
  updateRating = rating => {
    this.setState({ POINT: rating, ratingPopup: false });
    this.props.scrollToggle();
  };
    ...
    return (
      <View>
        <View style={{ borderBottomColor: "#dee2e6", borderBottomWidth: 1, padding: 20 }}>
          <Text style={{ color: "#212529", fontWeight: "bold", fontSize: 24, marginBottom: 10 }}>리뷰쓰기</Text>
          ...
          <View style={{ marginTop: 7, flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end" }}>
            <TextInput style={{ width: "85%" }} multiline={true} onChangeText={TEXT => this.setState({ TEXT })} placeholder="내용을 입력해주세요." />
        ...
        <View
          style={{
            ...
            display: this.state.ratingPopup ? "flex" : "none"
          }}
        >
          <Rating startingValue={this.state.POINT} fractions={1} onFinishRating={this.updateRating} imageSize={50} />
        </View>
      </View>
    );
  }
