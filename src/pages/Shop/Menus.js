      ...
      <View>
        ...
        >
          ...
          <View style={{ marginLeft: 20 }}>
            <Text style={{ color: "#212529", fontSize: textResizing(20), lineHeight: 35 }}>{this.props.MENUNAME}</Text>
            <View style={[{ flexDirection: "row", alignItems: "center" }, this.props.sales[0] ? {} : { marginTop: 10 }]}>
              ...
              {this.props.sales[0] ? (
                <Text style={{ fontSize: textResizing(16), color: "#212529", fontWeight: "bold", marginLeft: 5, marginRight: 5 }}>
                  {accounting.formatMoney(this.props.sales[0].PRICE, { symbol: "원", format: "%v%s", precision: 0 })}
                </Text>
              ) : null}
            </View>
            {this.props.sales[0] ? <Text style={{ fontSize: textResizing(16), color: "#212529" }}>({this.props.sales[0].COUNT}개 남음)</Text> : null}
          </View>

          <View style={{ position: "absolute", right: 0 }}>
            <TouchableWithoutFeedback onPress={() => (this.props.SOLD ? null : this.props.addCart(this.props._id))}>
              <View>
                <Text style={{ fontWeight: "bold", fontSize: textResizing(22), textAlign: "center", color: this.props.SOLD ? "#adb5bd" : "#468ef7" }}>
