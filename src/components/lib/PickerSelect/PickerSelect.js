...
import { ColorPropType, Modal, Picker, Platform, StyleSheet, Text, TextInput, TouchableOpacity, TouchableWithoutFeedback, View } from "react-native";
...
export default class RNPickerSelect extends PureComponent {
  ...
  static handlePlaceholder({ placeholder }) {
    if (isEqual(placeholder, {})) return [];
    return [placeholder];
  }

  static getSelectedItem({ items, key, value }) {
    let idx = items.findIndex(item => (item.key && key ? isEqual(item.key, key) : isEqual(item.value, value)));
    if (idx === -1) idx = 0;

    return { selectedItem: items[idx], idx };
  }
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    const newItems = RNPickerSelect.handlePlaceholder({ placeholder: nextProps.placeholder }).concat(nextProps.items);
    const { selectedItem, idx } = RNPickerSelect.getSelectedItem({ items: newItems, key: nextProps.itemKey, value: nextProps.value });
    const selectedItemChanged = !isEqual(nextProps.value, undefined) && !isEqual(prevState.selectedItem, selectedItem);

    if (itemsChanged || selectedItemChanged) {
      if (selectedItemChanged) nextProps.onValueChange(selectedItem.value, idx);
  ...
  constructor(props) {
    ...
    const items = RNPickerSelect.handlePlaceholder({ placeholder: this.props.placeholder }).concat(this.props.items);
    const { selectedItem } = RNPickerSelect.getSelectedItem({ items, key: this.props.itemKey, value: this.props.value });
  ...
  getPlaceholderStyle = () => {
    ...
    if (!isEqual(placeholder, {}) && this.state.selectedItem.label === placeholder.label) return { color: placeholderTextColor };
    return {};
  };
  ...
  renderPickerItems = () => this.state.items.map(item => <Picker.Item label={item.label} value={item.value} key={item.key || item.label} color={item.color} />);

  renderDoneBar = () => {
    const { doneText, hideDoneBar, onUpArrow, onDownArrow, onDonePress, style } = this.props;

    if (hideDoneBar) return null;
  ...
  renderIOS = () => {
    ...
    return (
      <View style={[defaultStyles.viewContainer, style.viewContainer]}>
        <TouchableWithoutFeedback onPress={() => this.togglePicker(true)}>{this.renderTextInputOrChildren()}</TouchableWithoutFeedback>
        <Modal
        ...
        >
          ...
          <View style={[defaultStyles.modalViewBottom, style.modalViewBottom]}>
            <Picker testID="RNPickerSelectIOS" onValueChange={this.onValueChange} selectedValue={this.state.selectedItem.value} {...pickerProps}>
  ...
  renderAndroid = () => {
    ...
    if (children) return this.renderAndroidHeadless();

    return (
      <View style={[defaultStyles.viewContainer, style.viewContainer]}>
        <Picker
          style={[hideIcon ? { backgroundColor: "transparent" } : {}, style.inputAndroid, this.getPlaceholderStyle()]}
          testID="RNPickerSelectAndroid"
          enabled={!disabled}
          onValueChange={this.onValueChange}
          selectedValue={this.state.selectedItem.value}
...
const defaultStyles = StyleSheet.create({
  viewContainer: { alignSelf: "stretch" },
  chevronContainer: {
    flex: 1,
    flexDirection: "row",
    marginLeft: 15
